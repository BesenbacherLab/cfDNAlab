...
- Motif labels are sorted deterministically and stored as JSON attributes on the numeric `motif_index` array with `label_field = "motif"` and `labels` in count-column order.
