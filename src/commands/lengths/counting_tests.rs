mod tests_length_counting_1 {
    use super::super::{LengthAxis, LengthCounts};
    ...
    fn length_axis_rejects_edges_below_minimum_fragment_length() {
        let error = LengthAxis::new(vec![9, 10]).expect_err("length edges below 10 bp should fail");
    ...
    #[test]
    fn length_counts_collapse_rejects_empty_input() {
        let counters: Vec<LengthCounts> = Vec::new();

        let error = LengthCounts::collapse(&counters).expect_err("empty counters should fail");

        assert!(
            error
                .to_string()
                .contains("collapse requires at least one LengthCounts"),
            "unexpected error: {error}"
        );
    }

    #[test]
    fn length_counts_collapse_rejects_different_axes_with_same_width() {
        let left = LengthCounts::new(axis(vec![10, 20]));
        let right = LengthCounts::new(axis(vec![20, 30]));
        let counters = vec![left, right];

        let error = LengthCounts::collapse(&counters)
