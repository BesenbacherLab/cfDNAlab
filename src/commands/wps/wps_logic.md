...
For each genomic centre `c` we place a window of width `window_size` around that base:
...
The helper `push_range(diff, start, end, weight)` adds `weight` to every centre in `[start, end)`.
We then run a prefix sum to recover the actual per-centre counts.

Given a fragment `[start, end)` (half-open, so the rightmost included base is `end - 1`):

- **Protected range**  
  A centre is protected when `c - left_span >= start` and `c + right_span <= end`.  
...
- **Left endpoint range**  
  ...
  Integer centres therefore satisfy `start - right_span + 1 <= c <= start + left_span - 1`.  
...
- **Right endpoint range**  
  ...
  Integer centres satisfy `end - right_span + 1 <= c <= end + left_span - 2`.  
...
## Blacklist and Valid Centres

We build a mask per tile that marks centres we must skip:
...
- Any centre whose resulting window would touch a blacklisted base is masked out.
- Centres that would extend past the chromosome edges are masked out.
