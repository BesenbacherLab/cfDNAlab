...
/// Identify forward/reverse reads (return (forward, reverse)) if both are mapped/inward.
///
/// Parameters
/// ----------
/// a: &Record
...
///     `(forward, reverse)` or `None` if invalid (unmapped, different contigs, same strand).
fn oriented_pair_from_records<'a>(
    a: &'a Record,
    b: &'a Record,
) -> Option<(&'a Record, &'a Record)> {
    if a.is_unmapped() || b.is_unmapped() || a.tid() != b.tid() {
        return None;
    }
...
/// Identify forward/reverse from compact MinimalReadInfo.
