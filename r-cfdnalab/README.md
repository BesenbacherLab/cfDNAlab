...
For `"split"`, `"outside"`, and `"inside"`, these side frequencies are calculated from the loaded full-length reference k-mers. If the reference output was restricted by a motifs file, only k-mers in that file contribute to the correction. If `ends` used `--motifs-file`, pass the same file to `ref-kmers`.
